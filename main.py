...
input = get_user_input()    #Số lượng gen trong 1 cá thể
population_size = 6    #Khởi tạo số lượng các thể trong 1 quần thể là 6
...
word_dict = {}    # Đọc từng file từ điển CSV và lưu trữ các từ vào một từ điển
for word_type, file_path in word_types.items():
    word_dict[word_type] = read_word_csv(file_path)

classified_words = {word: [] for word in input}    # Phân loại từng từ
for word in input:
...
grammar_string_2= "\n".join([
    ...
    "VP -> VB | VB NVP | VB INN | RB VB NN | RB VB NVP | C NVP | VB NN | NN | ADJP VB",
    "NVP -> UN NN | VB VP | NN JJ | CD NN | CD NVP | RB JJ | NNN ADJP | NNN JJ ADJP | NNN",
    "NNN -> NN NN",
    "INN -> IN NN",
    "NUN -> NN UN",
    "UNN -> UN NN",
...
def initialize_individual():
    ...
    return list(np.random.permutation(input))
